...
import usePrice from '../utils/customHooks/usePrice';
...
export default function TradeItem({ ressource, action, money, height }) {
  ...
  const [price, setPrice, priceProps] = usePrice();
      ...
      <Stack direction='row' spacing={1} justifyContent='center' alignItems='center'>
        ...
        <div style={{ marginLeft: 'auto' }}>
          <ButtonGroup size='small' variant='outlined' style={{ height: '28px', marginTop: '4px' }}>
            <Button {...priceProps} value='moins'>
              <ArrowDropDownIcon />
            </Button>
            <Button style={{ width: '100px', color: 'white' }} {...priceProps} disabled>
              {counter}
            </Button>
            <Button {...priceProps} value='plus'>
              <ArrowDropUpIcon />
            </Button>
          </ButtonGroup>
        </div>
